...
from src.rlkit.torch.torch_rl_algorithm import TorchTrainer
...
class NeurIPS20SACEnsembleTrainer(TorchTrainer):
    def __init__(
        ...
        if self.use_automatic_entropy_tuning:
            ...
            for _ in range(self.num_ensemble):
                log_alpha = ptu.zeros(1, requires_grad=True, torch_device=self.device) # Add torch_device=self.device
    ...
    def train_from_torch(self, batch):
        ...
        for en_index in range(self.num_ensemble):
            ...
            if self.use_automatic_entropy_tuning:
                """
                    Alpha loss is modified to be consistent with orginal SAC paper and our SAC implementation
                    Original: -(self.log_alpha[en_index] * (log_pi + self.target_entropy).detach()) * mask
                    Modified: -(self.log_alpha[en_index].exp() * (log_pi + self.target_entropy).detach()) * mask
                """
                alpha = self.log_alpha[en_index].exp()
                alpha_loss = -(alpha * (log_pi + self.target_entropy).detach()) * mask
            ...
            policy_loss = (alpha.detach()*log_pi - q_new_actions - self.expl_gamma * std_Q) * mask * weight_actor_Q.detach()
            ...
            q_target = self.reward_scale * rewards + self.discount * target_q_values
            # Terminal flag is disabled since we consider continuing task
